...
# TO DO: 
# check DICS BF mapping function 
#
...
def make_DICS_diff_BF_map(subjectID, specEv, originalEpochs, src, trans, bem, fmin, fmax, fBins, startTime, endTime, 
    activeWindowOnset, baselineWindowOnset, BFduration):
    
    # Calculate the DICS beamformer map for a set of spectral events
    #
    # Notes:
    #   - uses only magnetometers to calculate the BF map
    #   - uses lower/upper bound frequencies and event onset times, rather than peaks
    #
    # Input Paramaters:
    #   subjectID = subjectID
    #   specEv = dataframe containing spectral events
    #   originalEpochs = epoch data from MNE python
    #   src = source space
    #   trans = MEG/MRI registration file
    #   bem = boundary element model - from mne.read_bem_solution
    #   startTime = earliest time that an event can start to be included [s]
    #   endTime = latest time that an event can start to be included [s]
    #   fmin = lower frequency bound for events
    #   fmax = upper frequency bound for events
    #   fBins = number of frequency bins for DICS (linear spacing)
    #   activeWindowOnset = start time of active window (wrt event onset time)
    #   baselineWindowOnset = start time of baseline window (wrt event onset time)
    #   BFduration = duration of BF windows in seconds

    # DICS Settings
    tmins = [activeWindowOnset, baselineWindowOnset] # Start of each window (active, baseline) relative to event onset time
    tstep = BFduration        # Duration of BF windows in seconds, based on distribution of event durations 
    numFreqBins = fbins  # linear spacing
    ...
    # Re-calculate epochs to have one per spectral event centred at t=0s
    epochList = []
    ...
    forward = mne.make_forward_solution(epochs.info,
                                        trans=transFif, src=src, bem=bem,
                                        meg=True, eeg=False)
